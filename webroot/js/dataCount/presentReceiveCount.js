...
$(function($) {
    jQuery(grid_selector).jqGrid({
        url: "presentReceiveCount",
        datatype: "json",
        height: 450,
        cellLayout:4,
        colNames: ['主键','礼品名称','图片','礼品领取数','礼品领取率'],
        colModel: [
            {name:'id',width:"10",index:'id',hidden:true,align:'center'},
            {name:'name',index:'name',width:"30",align:'center'},
            {name:'icon',index:'icon',width:"30",align:'center',formatter:getPicUrlMapping},
...
//查询按钮
function search(){
    var name = $("#name").val();//礼品名称
    var curpagenum = $(grid_selector).jqGrid('getGridParam', 'page');   //当前页码
    $(grid_selector).jqGrid("setGridParam",
        {url:"presentReceiveCount?name="+name, page:curpagenum, datatype:"json"}).trigger("reloadGrid");
