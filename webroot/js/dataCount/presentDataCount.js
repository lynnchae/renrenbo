...
$(function($) {
    ...
    jQuery(grid_selector).jqGrid({
        url: 'presentDataCount',
        datatype: "json",
        height: 450,
        cellLayout:4,
        colNames: ['主键','销量排名','礼物名称','图片', '礼物销量','总金额'],
        colModel: [
            {name: 'id', width: "10", index: 'id', hidden: true, align: 'center'},
            {name: 'ranking', index: 'ranking', align: 'center', width: "20"},
            {name: 'name', width: "10", index: 'name', align: 'center'},
            {name: 'icon', index: 'icon', align: 'center', width: "20",formatter:getPicUrlMapping},
            {name: 'num', index: 'num', align: 'center', width: "20"},
            {name: 'totalAmount', index: 'totalAmount', align: 'center', width: "25"}
        ],
        viewrecords: true,
        rowNum: 10,
        rowList: [10, 20, 30],
...
//查询按钮
function search(){
    var name = $("#name").val();//礼物名称
    alert(name);
    var curpagenum = $(grid_selector).jqGrid('getGridParam', 'page');   //当前页码
    $(grid_selector).jqGrid("setGridParam",
        {url:"presentDataCount?name="+name, page:curpagenum, datatype:"json"}).trigger("reloadGrid");
